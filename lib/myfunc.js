...
exports.protex = () => {
  ...
  const mainFilePath = path.join(__dirname, '../index.js'); // or your main file

  // Check LICENSE file (existing code)
  ...
  // NEW: Check main file for credit block
  if (!fs.existsSync(mainFilePath)) {
    for (let i = 0; i < 50; i++) {
      console.error(chalk.redBright('[ SYSTEM CRASH ] Main file not found!'));
    }
    process.exit(1);
  }

  const mainFileContent = fs.readFileSync(mainFilePath, 'utf-8');
  const expectedCreditBlock = `/* ═════════════════════════════════
 *  🚀  MALVIN KING TECH - YT
 * ═════════════════════════════════
 *  
 ...
 * ════════════════════════════════ */`;
  ...
  if (!mainFileContent.includes('MALVIN KING TECH') || !mainFileContent.includes('malvintech2')) {
    for (let i = 0; i < 50; i++) {
      console.error(chalk.redBright('[ SYSTEM CRASH ] Credit block has been removed from main file!'));
      console.error(chalk.redBright('[ SYSTEM CRASH ] Do not remove developer credits!'));
    }
    process.exit(1);
  }
};
